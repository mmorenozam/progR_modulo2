---
title: "Módulo 2: Introducción al `tidyverse`"
...
+-------------------------------------------------------------------------------------------------+------------+
| El operador de cascada de base R es `|>`                                                        | V/F        |
+-------------------------------------------------------------------------------------------------+------------+
| Este es un buen nombre para un objeto en R: `Ingresos Anuales`                                  | V/F        |
+-------------------------------------------------------------------------------------------------+------------+
| Cuando digo que una tabla está en formato largo, puedo decir que es rectangular                 | V/F        |
+-------------------------------------------------------------------------------------------------+------------+
| `Z` es una función, `Y` es un objeto. Con la expr. `Y(Z)` estoy evaluando `Y` en función de `Z` | V/F        |
+-------------------------------------------------------------------------------------------------+------------+
| La tabla de datos `msleep` se refiere a las horas de sueño de mamíferos                         | V/F        |
+-------------------------------------------------------------------------------------------------+------------+

## Autoevaluación \# 5 {.smaller visibility="uncounted"}

+------------------------------------------------------------------------------------------------------------+-----------+
| Estas expresiones dan el mismo resultado:                                                                  | V/F       |
|                                                                                                            |           |
| `msleep %>% filter(order == "Primates" | sleep_total >= 10)`                                               |           |
|                                                                                                            |           |
| `msleep %>% filter(order == "Primates", sleep_total >= 10)`                                                |           |
+------------------------------------------------------------------------------------------------------------+-----------+
| Esta expresión filtra la tabla `iris` excluyendo la categoría `virginica` de la columna `Species`          | V/F       |
|                                                                                                            |           |
| `iris %>% filter(Species != "virginica")`                                                                  |           |
+------------------------------------------------------------------------------------------------------------+-----------+
| En la anterior expresión **NO** estoy haciendo cambios en la tabla `iris`                                  | V/F       |
+------------------------------------------------------------------------------------------------------------+-----------+
| Deseo calcular la suma de `Petal.Width` por `Species` de la tabla iris. Es correcta la siguiente expresión | V/F       |
|                                                                                                            |           |
...
## Autoevaluación \# 6 {.smaller}

+-------------------------------------------------------------------------------+------------------------------------------------------------------------------------+
| La tabla de datos `airquality` trata de                                       | a\) Medidas de calidad de aire en Nueva York                                       |
|                                                                               |                                                                                    |
|                                                                               | b\) Medidas de contaminación de aire                                               |
|                                                                               |                                                                                    |
|                                                                               | c\) Medidas de presión atmosférica en UK                                           |
+-------------------------------------------------------------------------------+------------------------------------------------------------------------------------+
| ¿Por qué debemos cambiar `\` por `/` en R?                                    | a\) Da igual, no es necesario                                                      |
|                                                                               |                                                                                    |
|                                                                               | b\) Puede provocar errores porque `\` es usado para caracteres especiales          |
|                                                                               |                                                                                    |
|                                                                               | c\) Solo es necesario en Windows                                                   |
+-------------------------------------------------------------------------------+------------------------------------------------------------------------------------+
| En la función `write.csv`, si el argumento `row.names` no es seteado como `F` | a\) El archivo .csv creado contendrá una columna extra indicando el número de fila |
|                                                                               |                                                                                    |
...
## Autoevaluación \# 6 {.smaller .scrollable visibility="uncounted"}

`mtcars` es una tabla de base R.

|                           |     |
|---------------------------|-----|
| Esta tabla es rectangular | V/F |
...
+-----------------------------------------------------+-----------------------------------------------------------------------+
| En la línea 1 estamos creando una copia de `mtcars` | V/F                                                                   |
+-----------------------------------------------------+-----------------------------------------------------------------------+
| En la línea 3 estamos                               | a\) Cambiando los nombres de las columnas de `tabla_cars`             |
|                                                     |                                                                       |
|                                                     | b\) Extrayendo los nombres de las filas de `tabla_cars`               |
|                                                     |                                                                       |
|                                                     | c\) Creando una nueva variable con los nombres de las marcas de autos |
+-----------------------------------------------------+-----------------------------------------------------------------------+
| En las líneas 4 a 7 estamos                         | a\) Transformando de formato largo a ancho                            |
|                                                     |                                                                       |
|                                                     | b\) Transformando de formato ancho a largo                            |
|                                                     |                                                                       |
|                                                     | c\) Preparando la tabla para exportar                                 |
+-----------------------------------------------------+-----------------------------------------------------------------------+
| La expresión `-c("marca")` significa                | a\) Que estamos borrando la columna "marca" de `tabla_cars`           |
|                                                     |                                                                       |
